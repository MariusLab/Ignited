...
#include "JoltDestructibleCompoundComponent.h"
...
UJoltDestructibleCompoundComponent::UJoltDestructibleCompoundComponent()
{
	bHiddenInGame = false;
...
void UJoltDestructibleCompoundComponent::DestroyAndSeparateIntoParts(const Vec3& Impulse, const Vec3& ImpactPoint)
{
	auto JoltSubsystem = GetWorld()->GetSubsystem<UJoltSubsystem>();

	for (UJoltBodyComponent* BodyComponent : DestructibleBodyComponents)
	{
		const int BodyIndex = BodyComponent->GetBodyID().GetIndex();
		JoltSubsystem->SetMotionType(BodyIndex, EMotionType::Dynamic);
		JoltSubsystem->SetObjectLayer(BodyIndex, EObjectLayer::Debris);
		JoltSubsystem->GetBodyInterface()->AddImpulse(BodyID(BodyIndex), Impulse, ImpactPoint);

		//BodyComponent->SetHiddenInGame(false, true);
	}

	JoltSubsystem->SetObjectLayer(GetBodyID().GetIndex(), EObjectLayer::NoCollision);
	//SetHiddenInGame(true, true);
}

void UJoltDestructibleCompoundComponent::RollbackDestroy()
{
	auto JoltSubsystem = GetWorld()->GetSubsystem<UJoltSubsystem>();
	...
	{
		const int BodyIndex = BodyComponent->GetBodyID().GetIndex();
		JoltSubsystem->SetMotionType(BodyIndex, EMotionType::Static);
		JoltSubsystem->SetObjectLayer(BodyIndex, EObjectLayer::NoCollision);
		
		//BodyComponent->SetHiddenInGame(true, true);
	}

	JoltSubsystem->SetObjectLayer(GetBodyID().GetIndex(), PhysicsProperties.Layer);
	//SetHiddenInGame(false, true);
}

void UJoltDestructibleCompoundComponent::AddJoltBodyComponent(UJoltBodyComponent* JoltBodyComponent)
{
	AddDestructibleComponent(JoltBodyComponent);
...
void UJoltDestructibleCompoundComponent::CopyRootBodyComponentProperties(UJoltBodyComponent* JoltBodyComponent)
{
...
void UJoltDestructibleCompoundComponent::InitBody(const FJoltBodyData& JoltBodyData)
{
...
void UJoltDestructibleCompoundComponent::OnRender()
{
...
const TArray<TJoltBodySettings>& UJoltDestructibleCompoundComponent::GetJoltBodiesSettings() const
{
	return JoltBodiesSettings;
}

void UJoltDestructibleCompoundComponent::AddDestructibleComponent(UJoltBodyComponent* JoltBodyComponent)
{
	//TODO: Then in rollback just switch between the static compound to dynamic falling pieces. We'll need to save/load bDestroyed or smth like that to know hwich object layer to set
	...
	auto SpawnedBodyActor = Cast<AJoltBodyActor>(GetWorld()->SpawnActor(AJoltBodyActor::StaticClass(), &GetComponentTransform()));
	check(SpawnedBodyActor);

	SpawnedBodyActor->BodyName = JoltBodyComponent->BodyName;

	//JoltBodyComponent->SetHiddenInGame(true, true);

	JoltBodyComponent->DetachFromComponent(FDetachmentTransformRules::KeepWorldTransform);
	LocalPlayerController->MoveComponentHierarchyToAnotherActor(JoltBodyComponent, SpawnedBodyActor);
	SpawnedBodyActor->SetRootComponent(JoltBodyComponent);

	LocalPlayerController->AddToCurrentStageJoltBodyActors(SpawnedBodyActor);

	JoltBodyComponent->bAllowDynamicOrKinematic = true;
	const int SpawnedBodyIndex = SpawnerSubsystem->AddJoltBody(SpawnedBodyActor);

	auto JoltSubsystem = GetWorld()->GetSubsystem<UJoltSubsystem>();
	JoltSubsystem->SetObjectLayer(SpawnedBodyIndex, EObjectLayer::NoCollision);
	JoltSubsystem->SetMotionType(SpawnedBodyIndex, EMotionType::Static);

	DestructibleBodyComponents.AddUnique(JoltBodyComponent);
}

#ifdef MY_DEBUG_RENDERING
FBoxSphereBounds UJoltDestructibleCompoundComponent::CalcBounds(const FTransform& LocalToWorld) const
{
	return FBoxSphereBounds( FVector::ZeroVector, FVector(100000.f), 100000.f).TransformBy(LocalToWorld);
}

FPrimitiveSceneProxy* UJoltDestructibleCompoundComponent::CreateSceneProxy()
{
	...
	public:
		...
		FBoxSceneProxy(UJoltDestructibleCompoundComponent* InComponent)
